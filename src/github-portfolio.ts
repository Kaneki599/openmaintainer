...
export interface GitHubPortfolioReport {
  ...
  totals: { repositories: number; healthy: number; attention: number; critical: number; errors: number; warnings: number; info: number };
}
...
export async function analyzeGitHubOwner(owner: string, options: GitHubPortfolioOptions = {}): Promise<GitHubPortfolioReport> {
  ...
  for (const repository of repositories) results.push(await analyzeRepository(owner, repository, options));
...
export function formatGitHubPortfolioHtml(report: GitHubPortfolioReport): string {
  const rows = report.repositories.map((repo) => `<tr><td><a href="${escapeHtml(repo.url)}">${escapeHtml(repo.name)}</a>${repo.archived ? " <small>archived</small>" : ""}</td><td><span class="status ${repo.status}">${repo.status}</span></td><td>${repo.score}</td><td>${repo.summary.errors}</td><td>${repo.summary.warnings}</td><td>${repo.summary.info}</td></tr>`).join("");
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>OpenMaintainer portfolio</title><style>:root{color-scheme:dark;font-family:Inter,system-ui;background:#07111f;color:#e5e7eb}body{margin:0;padding:32px}.shell{max-width:1100px;margin:auto}.hero{background:linear-gradient(135deg,#172554,#581c87);padding:28px;border-radius:24px}.metrics{display:flex;gap:12px;flex-wrap:wrap}.metrics span,.status{padding:7px 11px;border-radius:999px;font-weight:700;background:#172033}.critical{background:#7f1d1d}.attention{background:#78350f}.healthy{background:#14532d}table{margin-top:24px;width:100%;border-collapse:collapse;background:#0f172a;border-radius:18px;overflow:hidden}th,td{text-align:left;padding:14px;border-bottom:1px solid #293548}a{color:#67e8f9}@media(max-width:700px){body{padding:12px}table{display:block;overflow:auto}}</style></head><body><main class="shell"><section class="hero"><h1>${escapeHtml(report.owner)} repository portfolio</h1><div class="metrics"><span>${report.totals.repositories} repositories</span><span>${report.totals.critical} critical</span><span>${report.totals.attention} need attention</span><span>${report.totals.healthy} healthy</span></div></section><table><thead><tr><th>Repository</th><th>Status</th><th>Score</th><th>Errors</th><th>Warnings</th><th>Info</th></tr></thead><tbody>${rows}</tbody></table></main></body></html>\n`;
}
...
async function listRepositories(owner: string, options: GitHubPortfolioOptions): Promise<GitHubRepository[]> {
  ...
  for (let page = 1; repositories.length < limit; page += 1) {
    ...
    if (!response.ok) throw new Error(`Could not list repositories for ${owner}: GitHub returned ${response.status}.`);
...
async function analyzeRepository(owner: string, repository: GitHubRepository, options: GitHubPortfolioOptions): Promise<GitHubRepositoryResult> {
  ...
  try {
    ...
    if (!treeResponse.ok) throw new Error(`Could not read ${owner}/${repository.name}: GitHub returned ${treeResponse.status}.`);
